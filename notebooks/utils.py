...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prefixes_to_filter = [
      'Transfer','Salary','Direct Credit', 'Wdl ATM CBA', 'Home Loan Pymt',
      'Refund Purchase Medicare Benefit','Refund Purchase MEDICARE BENEFIT', 
...
def deduplicate_transactions(df: pd.DataFrame) -> pd.DataFrame:
    tdf = df.groupby(by=['date','desc', 'amount', 'balance']).agg(
      count_desc=('desc', 'count'),
      dist_files=('filename', 'nunique')  
    ).reset_index()

    print(f"Found {len(tdf[tdf.count_desc > 1])} duplicated entries \nand {len(tdf[(tdf.count_desc > 1) & (tdf.dist_files == 1)])} duplicates on the same day from the same file.")
...
def classify_transaction(transaction_description: str):
    ...
    # Hard coded rules
    if not transaction_description.startswith('COLES EXPRESS') and any([transaction_description.startswith(k) for k in ['COLES','WOOLWORTHS','RITCHIES SUPA IGA', 'THE ESSENTIAL INGRED', 'BWS LIQUOR', 'HARRIS FARM MARKETS']]):
        output = 'Groceries'
    elif transaction_description.startswith('Loan Repayment LN REPAY') or 'CommInsure' in transaction_description:
        output = 'HomeLoan'
    elif any([transaction_description.startswith(k) for k in ['COLES EXPRESS','7-ELEVEN','BP ','CALTEX','METRO PETROLEUM','SHELL','AMPOL', 'Enhance Neath Service']]):
        output = 'Fuel'
    else:
...
def keyword_classifier(transaction_description: str):
    labelled_keywords = {
        ...
        'Utility': ['BPAY'],
        'Health/Pharmacy': [
            'Medical','CHEMIST','PHARMACY', 'HUMMINGBIRD',
            'COUNSELLIN','DOCS MEGASAVE', 'Hamilton Doctors', 
            'THE GOOD DENTIST', 'NOVAHEALTH', 'PRICELINE',
            'SKINTIFIX','CHARLESTOWN FAMILY MED','WENT PHARM MOSCHAKIS',
            'PLINEPH', 'PLINE PH'
        ],
        'Parking': ['EASYPARK','ParkPay', 'WESTFIELD CHATSWOOD', 'WESTFIELD KOTARA'],
        'Newspaper': ['ACM RURAL PRESS'],
        'Cafe': [
            'EQUIUM', 'ONYX ESPRESSO', 'HUBRO', 
            'KAWUL','BOCADOS','Cafe','T MY ENTERPRISE PL MERRIWA', 
            'ALCMARIA PTY LTD CHATSWOOD', 'SOUL ORIGIN', 'AUTUMN ROOMS',
            'KAROO & CO'
        ],
        'Takeaway/Fastfood': [ 'SUSHI', 'FAHS KITCHEN', 'OLIVERS', 'MCDONALDS', 'Subway', 'DUNEDOO PIE', 'BAKERS DELIGHT'],
        'Home/Garden/Office': ['JB HI', 'OFFICEWORKS', 'BUNNINGS', 'BIG W', 'TARGET', 'MISTER MINT','POST WALLSEND', 'NewsXpress'],
        'Pets/Vet': ['Veterinary']
    }
